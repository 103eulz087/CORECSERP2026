...
9. `sp_Payment_PostBankReconEntry` is idempotent by (`ReferenceNo`, `AccountCode`, `ItemType`) — a second call with the same three values silently no-ops (`RETURN`, no insert, no error). If a posting SP calls it more than once for the same voucher against the same bank account (e.g. one call for invoice-driven cash, another for an unrelated manual GL line that happens to hit the same account), the second call is silently dropped and that reconciling entry is permanently missing with no error surfaced anywhere. Combine/net all cash movement per account into ONE call before invoking it — never call it once per source. Hit and fixed in `sp_PostVoucherManual` (`VoucheringManualFrm`).
10. A per-invoice/per-line "gross must equal the caller's own numbers" check is not the same as "gross must equal the current database state" — always re-derive the settlement amount from a freshly-read (and `UPDLOCK`'d, to close the race) balance, not from whatever the client passed in, even if the client's number is internally consistent with its own other numbers. A stale client-side Balance (grid loaded, then another payment posted against the same invoice before Submit) can pass every client-side/aggregate check and still leave a phantom residual balance or write off an uncompensated surplus. Hit and fixed in `sp_PostVoucherManual`'s Gross-vs-fresh-Balance assertion.
...
**Grid footer/totals band styling.** Reference implementation: `POS/POSSalesReportDevEx.cs`'s SalesTransactionSummary tab (`gridView1`/`gridView2`, wired in the constructor). The standard "totals band" look for a grid's `FooterPanel` (grand totals) and, where grouping is used, `GroupFooter`/`GroupRow` (per-group subtotals): `Font("Tahoma", 9.75F, FontStyle.Bold)`, `ForeColor = Color.FromArgb(102, 60, 0)` (dark brown), `BackColor = Color.FromArgb(255, 244, 214)` (warm cream), with the matching `Options.UseFont`/`UseForeColor`/`UseBackColor = true` on each. Don't copy the Font/Color literals into a new module — call `Classes.DevXGridViewSettings.ApplyTotalsBandAppearance(view)` (added 2026-09-13, applied to `AccountingDevEx/ClientPaymentsDevExAcctg.cs`'s `gridView2`/`gridViewPaymentDetails`) once per `GridView`, same timing as the reference (constructor, or once in `Load`) — keeps every report/grid's totals band visually uniform, and a future palette change only needs to happen in that one helper.

**Auto-mapped compound-entry posting pattern.** For a posting SP that needs to auto-generate some GL legs from config (rather than have the user manually type every line) while still allowing additional free-form manual lines in the same voucher, reference implementation: `sp_PostVoucherManual` (`AccountingDevEx/VoucheringManualFrm.cs`, see `SQL/2026-09-15_VoucheringManual_AutoAPTradeLegs.sql`). Shape:
- Resolve every auto-generated leg's account code from `JournalEntryMapping` (e.g. `VOUCHER-MANUAL-FXVAR`, `VOUCHER-MANUAL-APTRADE`) — never hardcode a GL account in the proc body, even a "default" one; a per-row source column (like `ExpenseSummary.PayableAccountCode`) can still take priority over the mapping when it's genuinely more specific.
- Stage every leg — both auto-generated and user-typed — into a local `#Legs` temp table (`AccountCode, Debit, Credit`) as you build them, instead of inserting straight into `TicketDetails`.
- After all legs are staged, do ONE final `GROUP BY AccountCode` netting pass (`SUM(Debit)-SUM(Credit)` per account, skip any account that nets to exactly 0) before the single real `INSERT INTO TicketDetails` — produces a clean compound entry (one row per account actually touched) instead of a scattered row-per-source ticket.
- Per-source detail is NOT lost by netting — it still lives in its own dedicated column/table (e.g. `APPaymentDetails.Variance`, `SupplierLedger`) for audit trail; only the `TicketDetails`/GL view itself is compounded.
- Any shared "post a reconciling bank entry" helper (e.g. `sp_Payment_PostBankReconEntry`) must be called with the amount already netted per account too if it's idempotent by account — see Known Bug Pattern #9.
...
- **Databases (as of 2026-09-25):**
  - **`COREX001` = DEV.** It is the new DEV database, a live replica of STAGING, and it is what `HKCU\AAITCRE\ConnSettingsMain` points to. Make and test all changes here first.
  - **`CORECSJFC2026_STAGING` = STAGING.** Apply a change here only after the user confirms.
  - **`CORECSERP_002_DEV` = the old DEV.** No longer the target. Don't deploy to it or treat its data as current. Its data differs from COREX001; e.g. it has telegraphic vouchers that COREX001 and STAGING don't.
- All three are hosted on `corex.itcoreapps.com` (a single server, connected to directly by database name — no named instance/port needed). Credentials for this server are stored in the registry per the mechanism above (`HKCU\AAITCRE\ConnSettingsMain` for DEV/COREX001, `ConnSettingsServer` for STAGING) — never in this file or any other checked-in source.
- Before any DB check or deploy, read the `Initial Catalog` from the registry connection string instead of assuming a database name. A helper that forces a different catalog will silently query a database the app isn't using. That happened on 2026-09-25: checks against `CORECSERP_002_DEV` showed data the app, on COREX001, never saw.
...
lives in `docs/CLAUDE_WORKLOG.md` and is imported below, so a fresh session on any
machine starts with it. Update that file (not this one) as work progresses; keep it short.

@docs/CLAUDE_WORKLOG.md
