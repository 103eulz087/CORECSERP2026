SET NOCOUNT ON;
PRINT '=== sp_AddPaymentClient: restore +@Overpay in NetOverpay ===';
GO
EXEC sp_rename 'dbo.sp_AddPaymentClient', 'sp_AddPaymentClient_OLD_08022026134317';
GO
SET QUOTED_IDENTIFIER ON;
GO
-- ============================================================
--  sp_AddPaymentClient - FINAL VERSION
--  Project : CORECSJFC2026
--  Date    : 2026-06-27
-- ============================================================
-- CHANGELOG FROM PREVIOUS VERSION:
--
-- [1] @Overpay variable added - reads PaymentType='OVERPAY' rows
--     from ARPaymentDetails
--
-- [2] NET split into NET_OFFSET and NET_OVERPAY:
--     NET_OFFSET  = Gross - EWT - Discount - Offset  (client pays less)
--     NET_OVERPAY = Gross - EWT - Discount + Overpay (client pays more)
--
-- [3] Mnemonic matrix expanded to 12 scenarios covering all
--     combinations of EWT / Discount / Offset / Overpay
--
-- [4] #Computed Amount CASE handles all new AmountTypes
--
-- [5] IsConditional filter now includes HasOverpay
--
-- [6] TransactionChargeSales updates:
--     - AmountPaid = net cash received (computed per invoice)
--     - EWTAmount  = EWT per invoice
--     - DiscountAmount = Discount per invoice
--     - OffsetAmount is NOT updated - offset is a liability
--       clearance, not a deduction from the invoice balance.
--       Balance = TotalAmount - (AmountPaid + EWTAmount + DiscountAmount)
--     - Overpay is NOT stored on TCS either - it goes to
--       Other Income via the GL entry only.
--
-- [7] DIT bank recon insert uses correct net cash:
--     - OFFSET scenario: cash = @NetOffset
--     - OVERPAY scenario: cash = @NetOverpay
--     - Plain: cash = @Net
--
-- 2026-08-02: [8] FIX - @NetOverpay was missing "+ @Overpay" (it
--     had been commented out), so BankCash and the NET_OVERPAY GL
--     mapping amount silently excluded the overpay cash received.
-- ============================================================
...
BEGIN
   ...
   -------------------------------------------------------
    ...
    -------------------------------------------------------
   IF @PayType IN ('CASH', 'CHECK', 'ONLINE', 'ADVANCEPAYMENT')
    BEGIN
...
END
GO
PRINT 'DEPLOYMENT COMPLETE: sp_AddPaymentClient fixed, original preserved as sp_AddPaymentClient_OLD_08022026134317.';
